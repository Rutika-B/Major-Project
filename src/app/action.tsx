...
Besides that, you can also chat with users and do some calculations if needed.`,
        ...
        parameters: z.object({
          ...
          events: z.array(
            z.object({
              ...
              summary: z.string().describe('The description of the event'),
              url: z.string().describe('url of article'),
            }),
          ),
        }),
      },
  ...
  (await completion).onTextContent((content: string, isFinal: boolean) => {
  ...
  async function fetchStockPrice(symbol: string) {
    try {
      const response = await fetch(
        `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=6JUD7MJHGV1YNRKB`,
      );
  ...
  async function fetchStockNews(symbol: string) {
    try {
      const response = await fetch(
        `https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=${symbol}&apikey=6JUD7MJHGV1YNRKB`,
      );
      ...
      const extractedData = news.map(
        (each: {
          ...
          time_published: each.time_published,
        }),
      );
  ...
  (await completion).onFunctionCall('show_stock_price', async ({ symbol }) => {
    ...
    aiState.done([
      ...
      {
        role: 'function',
        name: 'show_stock_news',
        content: `[Price of ${symbol} = ${price}]`,
      },
    ]);
  });

  (await completion).onFunctionCall('list_stocks', async ({ stocks }) => {
    reply.update(
      <BotCard>
        <StocksSkeleton />
      </BotCard>,
  ...
  (await completion).onFunctionCall('get_events', async ({ symbol }) => {
  ...
  (await completion).onFunctionCall(
